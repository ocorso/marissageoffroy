	...
	*/
	...
	class pirenko_twitter_widget extends WP_Widget 
	{
		...
		function widget( $args, $instance ) 
		{
			...
			$rand_nbr=rand(1, 500);
			echo ("<div id='pirenko_tweets_".$rand_nbr."'>");
			...
			<script type="text/javascript">
				...
				{		
					jQuery.getJSON('https://api.twitter.com/1/statuses/user_timeline.json?screen_name=<?php echo $instance['screen_name']; ?>&count=<?php echo $instance['num_tweets']; ?>&exclude_replies=true&callback=?', function(data){
					jQuery.each(data, function(index, item)
					{
						profile_image_url= item.user.profile_image_url;
						jQuery('#pirenko_tweets_<?php echo $rand_nbr; ?>').append('<div class="tweet"><div class="tw_img"><img src="<?php echo (get_bloginfo('template_url'). '/inc/theme_widgets/pirenko-twitter/icons/'.$curr_options['icon_set'].'/twitter.png'); ?>" /></div><div class="tweet_body">' + item.text.linkify() + '<p><?php echo $about_text; ?> ' + relative_time(item.created_at) + '</div></p></div>');
					});
					jQuery('#twitter_avatar_img_<?php echo $rand_nbr; ?>').attr('src',profile_image_url);
			...
			<?php 
				...
				echo ("</div>");
			?>
			<div class="twitter_avatar" class="">
            	<a href="http://twitter.com/<?php echo $instance['screen_name']; ?>" target="_blank">
                	<img id="twitter_avatar_img_<?php echo $rand_nbr; ?>" class="twitter_avatar_img" src="" width="32" height="32"  />
                </a>
            </div>
            <?php
			if (isset($instance['follow_text']))
				...
				$follow_text="Follow me on Twitter"
				?>
            <a class="lk_text" href="http://twitter.com/<?php echo $instance['screen_name']; ?>" target="_blank">
