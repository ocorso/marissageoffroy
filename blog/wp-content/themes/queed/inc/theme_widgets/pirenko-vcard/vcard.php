<?php
	...
	*/
	...
	function load_pirenko_vcard() {
		register_widget('queed_Vcard_Widget');
	}
class queed_Vcard_Widget extends WP_Widget 
{
  	function queed_Vcard_Widget() 
  	{
		$widget_ops = array('classname' => 'widget_queed_vcard', 'description' => __('Use this widget to add a vCard', 'queed'));
		$this->WP_Widget('widget_queed_vcard', __('Pirenko: vCard', 'queed'), $widget_ops);
		$this->alt_option_name = 'widget_queed_vcard';
	...
	{
		$cache = wp_cache_get('widget_queed_vcard', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('vCard', 'queed') : $instance['title'], $instance, $this->id_base);
    ...
    <style type="text/css">
		...
		.vcard .pir_home:before, 
		.vcard .pir_email:before, 
		.vcard .pir_fax:before, 
		.vcard .pir_phone:before {
			font-size:19px;	
		}
	</style>
    <div class="vcard">
        <?php echo home_icon_shortcode(); ?>
      	<div class="adr" style="">
        	<span class="ttl"><?php echo $instance['company_name']; ?></span><br>
        ...
        <div class="tel" style=""><?php echo $instance['tel']; ?></div>
    ...
    wp_cache_set('widget_queed_vcard', $cache, 'widget');
}
...
function update($new_instance, $old_instance) 
{
    ...
    if (isset($alloptions['widget_queed_vcard'])) 
	{
      	delete_option('widget_queed_vcard');
    }
	...
	function flush_widget_cache() 
  	{
    	wp_cache_delete('widget_queed_vcard', 'widget');
  	}
	...
	function form($instance) 
  	{
    	...
    	$email = isset($instance['email']) ? esc_attr($instance['email']) : '';
  		?>
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title (optional):', 'queed'); ?></label>
      		<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
    	</p>
        <p>
      		<label for="<?php echo esc_attr($this->get_field_id('company_name')); ?>"><?php _e('Company Name:', 'queed'); ?></label>
    	...
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('street_address')); ?>"><?php _e('Street Address:', 'queed'); ?></label>
    	...
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('locality')); ?>"><?php _e('City/Locality:', 'queed'); ?></label>
    	...
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('region')); ?>"><?php _e('State/Region:', 'queed'); ?></label>
    	...
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('postal_code')); ?>"><?php _e('Zipcode/Postal Code:', 'queed'); ?></label>
    	...
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('tel')); ?>"><?php _e('Telephone:', 'queed'); ?></label>
    	...
    	<p>
      		<label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php _e('Email:', 'queed'); ?></label>
