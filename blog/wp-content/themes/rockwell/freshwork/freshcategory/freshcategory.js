...
var actual_cat = null;
    ...
    $('#cat-list li').click(function(){
        ...
        actual_cat = $(this).attr('rel');
        load_form( actual_cat );
        //alert(category_data_holder[actual_cat]['cat_template'] );

            if( category_data_holder[actual_cat]['cat_template'] != '' )
            {
                 //alert('xxx');
                var new_radio_cat = $('#category-options').find('input[value='+category_data_holder[actual_cat]['cat_template']+']');
            ...
            //single
             if( single_data_holder[actual_cat]['single_template'] != '')
            {
                var new_radio_single = $('#single-options').find('input[value='+single_data_holder[actual_cat]['single_template']+']');
...
function load_form( cat_name ) {
    act_cat = cat_name;

    $('#category-options-additional').find('input').each(function(){
        var input_value = category_data_holder[act_cat][$(this).attr('name')];
        if($(this).attr('type') == 'checkbox')
        {
            ...
            if (input_value == 'false') input_value = 0;
    ...
    $('#single-options-additional').find('input').each(function(){
        var input_value = single_data_holder[act_cat][$(this).attr('name')];
        if($(this).attr('type') == 'checkbox')
        {
            if(parseInt(input_value) == 1)
                $(this).attr('checked', true);
...
function save_form( ) {
    ...
    $('#category-options-additional').find('input').each(function(){
        ...
        if($(this).attr('type') == 'checkbox')
        {
            ...
            output_value =  $(this).attr('value');
        }
       // alert ($(this).attr('name') + $(this).attr('checked') );
        // alert( output_value );
        category_data_holder[act_cat][$(this).attr('name')] = output_value;
    });
    $('#category-options-additional').find('select').each(function(){

        category_data_holder[act_cat][$(this).attr('name')] = $(this).attr('value');
    });
    //alert('val:'+ $('#category-options').find('input[name=category-template]:checked').val());
   category_data_holder[act_cat]['cat_template'] =  $('#category-options').find('input[name=category-template]:checked').val() ;
  //alert(act_cat);
    $('#single-options-additional').find('input').each(function(){
        var output_value = null;
        if($(this).attr('type') == 'checkbox')
        ...
        single_data_holder[act_cat][$(this).attr('name')] = output_value;
    });
         single_data_holder[act_cat]['single_template']  =  $('#single-options').find('input[name=single-template]:checked').val() ;
 ...
 $('.cat_apply').click(function(){
    ...
    $('.button-primary').click(function(){ if(actual_cat != null) save_form(); });
   ...
   //freshcategory_tooltip
    ...
    function reset_category()
    {
            ...
            $('input[value=blog-cat-12]').attr('checked','checked');
    }
    ...
    function reset_single()
    {
            ...
            $('input[value=blog-single-4]').attr('checked','checked');
           //     alert('dick head');
    }
