 ...
 *
 * /framework/admin/classes/class.options_machine.php
 * Version of this file : 1.7131
 *
 */
?>
<?php
...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'range':
					...
					$output .= '" onchange="printValue(\'' . esc_attr( $value['id'] ) . '\',\'' . esc_attr( $value['id'] ) . '_value\')" /> <input id="' . esc_attr( $value['id'] ) . '_value" type="text" class="range_value" />&nbsp; '. $value['unit'];
				break;
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','alephtheme').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		//initialize all vars
		$slidevars = array('order','slidelayout','showtext','contentstyle','contentbackground','title','caption','positiontext','showbutton','buttontext','buttonlink','buttonstyle','mediatype','imageurl','imagebg','videoprovider','videourl');
		...
		$slider .= '<div class="sub_section">';
			...
			$slider .='<div class="fold1 hidden">';
				...
				$slider .= '<div class="select_wrapper">';
				$position_options = array('topleft'=>'topLeft','topcenter'=>'topCenter','topright'=>'topRight',
				...
				// Button link
				$slider .= '<label>Button link (will open in new window)</label>';
				...
				$slider .= '<div class="select_wrapper">';
				$button_options = array('default'=>'Default','primary'=>'Primary',
