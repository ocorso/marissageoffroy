...
(function(jQuery) {
	...
	jQuery.fn.custSelectBox = function(options){
			...
			if(jQuery(this).attr("class").indexOf("disabled") < 0)
			{
				...
				if(boxtype.indexOf("selectboxoptions_radio") >-1)
				{
					...
					else
					{
					  ...
					  jQuery(this).parent().parent().parent().find(".selectboxopen").find("li").html(jQuery(this).html() );
					// neco
					 // alert("pica");
				}
				else //checkbox
      ...
      var thisElement = jQuery(opts.wrappername);
      	jQuery(".select_wrap").find(".selectboxopen").removeClass(classselectboxopen).addClass(classselectbox);
      	open = false;
					//slides the options up
					jQuery(this).parent().parent().parent().find(selectboxoptions_wrap).slideUp("normal");
